...
class AAE(nn.Module):
    ...
    def forward(
        self, depths_in, tnfs_in, z_prior, y_prior, labels_prior, labels_latent
    ):
        ...
        g_loss_adv_z = torch.nn.functional.binary_cross_entropy(
            d_z_latent, labels_prior
        )
        g_loss_adv_y = torch.nn.functional.binary_cross_entropy(
            d_y_latent, labels_prior
        )
        ...
        prior_z_loss = torch.nn.functional.binary_cross_entropy(
            self.discriminator_z(z_prior), labels_prior
        )
        latent_z_loss = torch.nn.functional.binary_cross_entropy(
            self.discriminator_z(z_latent.detach()), labels_latent
        )

        d_z_loss = 0.5 * (prior_z_loss + latent_z_loss)

        # Measure discriminator's ability to classify real from generated samples
        prior_y_loss = torch.nn.functional.binary_cross_entropy(
            self.discriminator_y(y_prior), labels_prior
        )
        latent_y_loss = torch.nn.functional.binary_cross_entropy(
            self.discriminator_y(y_latent.detach()), labels_latent
        )
    ...
    def trainmodel(
        self, data_loader, nepochs, batchsteps, T, lr, modelfile=None, logfile=None
    ):
        ...
        for epoch_i in range(nepochs):
            ...
            for depths_in, tnfs_in in data_loader:
                ...
                # Forward pass
                (vae_loss, ce, sse, ed_loss, d_z_loss, d_y_loss,) = self(
                    depths_in, tnfs_in, z_prior, y_prior, labels_prior, labels_latent
                )
            ...
            if logfile is not None:
                print(
                    "\tEpoch: {}\t Loss Enc/Dec: {:.6f}\t Rec. loss: {:.4f}\t CE: {:.4f}\tSSE: {:.4f}\t Dl loss: {:.7f}\t Dy loss: {:.6f}\t Batchsize: {}\t Epoch time(min): {: .4}".format(
                        epoch_i + 1,
